...
test('normal play navigation exposes the eight honest on-device modes', () => {
  assert.deepEqual(
    FEATURED_MODES.map((mode) => mode.id),
    ['obstacle', 'tag', 'balloon', 'skate', 'karting', 'show67', 'squid67', 'creator'],
  );
...
test('playable routes load on demand and share concurrent registration work', async () => {
  assert.deepEqual(GAME_ROUTE_IDS, ['tag', 'balloon', 'obstacle', 'skate', 'karting', 'creator', 'show67', 'squid67']);
...
test('town lobby stays inside the draw budget at both quality tiers', () => {
  ...
  // 226k triangles, and each venue room adds eleven to sixteen. With four
  // rooms (pool, club, boutique, arcade) the town measures 161, so 170 is the
  // ceiling — still nothing to a modern GPU — and the guardrail keeps
  // catching real regressions instead of blocking a room.
  assert.ok(high.estimatedDraws <= 170);
